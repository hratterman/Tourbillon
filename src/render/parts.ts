...
export const GEO = {
  ...
  hourRackPivot: LAYOUT.RACK_GEO.hour.pivot,
  quarterRackPivot: LAYOUT.RACK_GEO.quarter.pivot,
  minuteRackPivot: LAYOUT.RACK_GEO.minute.pivot,
  hammerLow: LAYOUT.HAMMER_LOW.pivot,
  hammerHigh: LAYOUT.HAMMER_HIGH.pivot,
  gongAnchor: { x: 150, y: -5 },
  allOrNothing: LAYOUT.AON_PIVOT,
  crown: { x: 233, y: 0 },
  slide: { x: -208, y: 0 },
  dialR: 172,
  apertureR: 54,
...
const ZPOS: Record<string, number> = {
  ...
  trainBridge: 0,
  tourbillonBridge: 0,
  strikeBridge: (LAYOUT.STRIKE_BRIDGE.z0 + LAYOUT.STRIKE_BRIDGE.z1) / 2,
  barrel: 8,
  centerWheel: 10.25,
  thirdWheel: 8.5,
  fixedFourth: 16,
  ...
  strikeTrain: 28,
  flyGovernor: 30.9,
  allOrNothing: 25.5, // hook plane ducks under the gong tubes
  gongLow: LAYOUT.GONG_LOW.z,
  gongHigh: LAYOUT.GONG_HIGH.z,
  hammerLow: LAYOUT.HAMMER_LOW.z,
  hammerHigh: LAYOUT.HAMMER_HIGH.z,
  dial: 43,
  hourHand: 46,
  minuteHand: 48.7,
  caseband: 17,
...
export function buildParts(): Part[] {
  ...
  const parts: Omit<Part, 'zPos'>[] = [
    ...
    },
    {
      ...
      z: 14,
      assembled: fixed(0, 0),
      explodeDir: { x: 0, y: 0, z: -1 }, // lifts off the back, like a real strip-down
      hitRadius: 80,
      liveState: () => [
        'carries the barrel arbor, centre & transfer bearings',
        'ratchet, crown wheel and click ride its back face',
      ],
    },
    // -------------------------------------------------- dial side (z 30+)
    {
    ...
    },
    {
      ...
      layer: 'dialside',
      z: 40.5,
      assembled: fixed(g.strikeTrain.x, g.strikeTrain.y),
      explodeDir: DIR_UP,
      hitRadius: 40,
      liveState: () => ['upper pivots for the gathering staff, intermediate and governor'],
    },
    {
      id: 'allOrNothing',
